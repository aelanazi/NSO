...
def is_svlan_id_in_use(root, service, **link):
    ...
    # a will be placeholder list to hold port and svlan which is extracted from dictionary link.
    a = []
    a.append(link['pe_port_1'])
    a.append(link['svlan_id'])
    # values is a string to join port and svland using . i.e. 0/0.100
    ...
    if link['pe_port_type'] == "Bundle-Ether":
        a = []
        a.append(link['INT-TYPE'])
        a.append(link['svlan_id'])
        # values is a string to join port and svland using . i.e. 0/0.100
        values = '.'.join(str(v) for v in a)
    """
    bundle_config = root.ncs__devices.device[device].config.cisco_ios_xr__interface[
        'Bundle-Ether-subinterface']['Bundle-Ether']
    if values in bundle_config:
        result = True

    elif link['pe_port_type'] == "GigabitEthernet":
        gig_config = root.ncs__devices.device[device].config.cisco_ios_xr__interface[
            'GigabitEthernet-subinterface']['GigabitEthernet']
